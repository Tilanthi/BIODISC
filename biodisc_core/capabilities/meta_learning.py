...
SwarmMetaLearner = MetaLearner
TaskCharacteristics = TaskSignature
DomainAdapter = HyperparameterAdapter

# TaskComplexity as enum (placeholder)
from enum import Enum
...
# TaskEmbedder placeholder class
class TaskEmbedder:
    """Embed tasks into feature space for similarity matching"""
    def __init__(self, dim: int = 128):
        self.dim = dim

    def embed(self, task_signature: TaskSignature) -> np.ndarray:
        """Embed task signature into vector space"""
        return task_signature.to_vector()
