...
class SafeModificationApplier:
    ...
    def apply(
        ...
        if sandbox_test:
            ...
            if not test_success:
                return False, {'error': 'Sandbox test failed'}

        # Apply the modification
        self.applied_modifications.append(proposal)
        self.rollback_stack.append(snapshot_id)

        return True, {'snapshot_id': snapshot_id, 'applied_at': time.time()}

    def _create_snapshot(self, state: Dict[str, Any], proposal_id: str) -> str:
        """Create a snapshot of current state."""
        snapshot_id = f"snapshot_{proposal_id}_{int(time.time())}"
        self.state_snapshots[snapshot_id] = state.copy()
        return snapshot_id
...
class CognitiveSelfModificationSystem:
    """
    Unified cognitive self-modification system.

    Integrates performance monitoring, bottleneck detection,
    strategy evaluation, modification engine, and safe application.
    ...
    def apply_modification(self, proposal: ModificationProposal, current_state: Dict[str, Any]) -> Tuple[bool, Dict[str, Any]]:
        """Apply modification with safety checks."""
        success, result = self.safe_applier.apply(proposal, current_state)
        if success:
            self.applied_modifications.append(proposal)
...
def create_safe_applier() -> SafeModificationApplier:
    ...
    return SafeModificationApplier()
