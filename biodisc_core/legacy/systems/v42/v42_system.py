...
# Factory functions for creating V42 systems
def create_v42_standard(**kwargs):
    """Create a standard V42 system with default configuration."""
    config = V42Config(
        mode=V42Mode.STANDARD,
        enable_test_time_search=True,
        enable_adaptive_compute=True,
        enable_self_consistency=True
    )
    return V42CompleteSystem(config)

def create_v42_fast(**kwargs):
    """Create a fast V42 system optimized for speed."""
    config = V42Config(
        mode=V42Mode.FAST,
        enable_test_time_search=False,
        enable_adaptive_compute=True,
        enable_self_consistency=False
    )
    return V42CompleteSystem(config)

def create_v42_deep(**kwargs):
    """Create a deep V42 system with maximum analysis."""
    config = V42Config(
        mode=V42Mode.DEEP,
        enable_test_time_search=True,
        enable_adaptive_compute=True,
...
def create_v42_gpqa(**kwargs):
    """Create a V42 system optimized for GPQA tasks."""
    config = V42Config(
        mode=V42Mode.GPQA,
        enable_test_time_search=True,
        enable_adaptive_compute=True,
