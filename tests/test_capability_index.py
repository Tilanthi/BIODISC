...
def test_index_is_honestly_low_when_replication_is_zero():
    ...
    idx = compute_capability_index(
        verdict_summary={"total_candidates": 100, "buckets": {"stored": 0, "quarantined": 10}},
        effectiveness=20.0)
...
def test_index_rises_when_replication_succeeds():
    idx = compute_capability_index(
        verdict_summary={"total_candidates": 100, "buckets": {"stored": 5, "quarantined": 5}},
        effectiveness=20.0)
...
def test_index_ingests_a_lowering_input():
    ...
    high = compute_capability_index(
        {"total_candidates": 100, "buckets": {"stored": 0, "quarantined": 10}}, effectiveness=80.0)
    low = compute_capability_index(
        {"total_candidates": 100, "buckets": {"stored": 0, "quarantined": 10}}, effectiveness=0.0)
...
def test_replication_dominates_throughput():
    ...
    high_volume_no_replication = compute_capability_index(
        {"total_candidates": 1000, "buckets": {"stored": 0, "quarantined": 500}}, effectiveness=50.0)
    low_volume_some_replication = compute_capability_index(
        {"total_candidates": 10, "buckets": {"stored": 2, "quarantined": 2}}, effectiveness=50.0)
