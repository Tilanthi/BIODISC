#!/usr/bin/env python3
"""
Generate Discovery-First PDF manuscript - Final Version
...
# Paths
MANUSCRIPT_PATH = "/Users/gjw255/astrodata/SWARM/BIODISC/bacterial_cycle/Origins_regulation_bacterial_cellcycle_RESTRUCTURED_FINAL.md"
...
def markdown_to_pdf_elements(md_text, figures_dir):
    ...
    for i, line in enumerate(lines):
        ...
        # Italic formatting (but be careful with mathematical expressions)
        # Only italicize words, not single letters that might be variables
        text = re.sub(r'\*([a-zA-Z]{3,}?)\*', r'<i>\1</i>', text)
...
def parse_table(lines):
    ...
    for col_idx in range(max_cols):
        ...
        col_widths.append(max(0.8*inch, min(max_len * 0.08 * inch, 2.5*inch)))
...
def main():
    ...
    print("  ✓ Two-dimensional matrix framework: Section 2 (discovery-first)")
    print("  ✓ Discovery analyses D1-D5: Section 3 (prominent)")
    print("  ✓ Physical Constraints: Section 4 (condensed but comprehensive)")
    print("  ✓ Molecular Regulation: Section 5 (condensed but comprehensive)")
