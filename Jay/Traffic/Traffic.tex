...
\date{Version 1.11. Accepted XXX. Received YYY; in original form ZZZ.}
...
urgency?  We examine a stock-exchange-style
\emph{board} of decaying information, read by every participant and
commanded by none, through a five-phase study: problem
...
no-shows without overbooking exposure; a perfect-information
central assigner at matched cadence achieves the same throughput and
wait but wins on wait fairness.  Phase~V concludes that the protocol
is a fit for a bounded field trial: it concedes nothing to central
optimisation on throughput, holds quality under failure, and keeps
per-participant compute independent of fleet size.
\end{abstract}
...
two vehicles holding the same stall-slot, enforced by minimal-data
computation rather than negotiation; a priority channel with anti-fraud
...
Our task is to attack it.  The study
...
Second, the parts are old: order books,
ground-delay programmes, overbooking, pheromone routing, tuple
spaces, and the OCPI roaming protocol each own a component.  The
composition --- decaying shared state as the reservation substrate ---
...
supplies the time dimension: a vehicle scores a station by the
...
the protocol is spatio-temporal.
...
re-optimised by edict.  E3 runs that comparison.
...
The board protocol occupies the resulting gap.  The industry
built the medium's transport layer; its semantics, and with them
failure-tolerant decentralised reservation, remain unbuilt.
...
while the board sees only reports, so any board advantage survives
an information handicap and any central advantage is
...
instead of a synthetic purity.  We did not tune away the protocol's
wait-fairness weakness reported in \S\ref{sec:results}: the
...
same cadence ($57{,}906$ served, $8.2$~min wait), although it sees
the world only through aged reports while central reads it
fresh.  Two qualifications.  (i) Central at a realistic
...
indistinguishable from the board in benign conditions, as expected:
the pheromone costs nothing when nothing rots.
...
the decision radius bounds the work, not the fleet size.  The
central
...
death instantly, strands nothing.  Decomposed, the board's survival
rests mostly on shared machinery:
...
own contribution is a margin rather than a rescue: $14\%$ fewer forced
...
rather than any cleverness, carries S2.  Central and nearest are
untouched by construction (one reads ground truth, the other reads
nothing).

\input{tab_E3_outage.tex}

\input{tab_E3_drop.tex}
...
behind stretched sessions.  Decay does not help here, by
...
\input{tab_E4_noshow.tex}

\input{tab_E4_longtail.tex}
...
($1.79\to3.15$~min; the fraud lands on the channel's intended
users, not the honest fleet) and throughput drops by
$552$ sessions/day.  Attestation converts
fraud from a system-integrity problem into a rounding error;
without it, the priority channel's value inverts for the very
...
moves).  The beam nudges rather than rescues: it prevents the
follow-the-leader pile-up at whatever single station a leader picks,
the failure mode the sketch imported from biological
trail formation.
...
and does not beat, a perfect-information central assigner on
throughput and wait at matched cadence, and \emph{loses} on wait
fairness (Jain $0.31$ vs $0.67$; E1).  It is not a better
optimiser, and should not be sold as one.
...
The first rung of the protocol needs no standardisation at
...
silent, and deployment that begins as a client-side reading of
feeds that already exist.

The biology contributed what the companion
programme predicted: a semantics rather than a metaphor.  The
...
Version 1.11; a text-only revision of 1.10 (results unchanged).
Results and text version together in future updates.
...
feed happened to be available.  The trial continues with a sharper
requirement: rung 1 needs OCPI-grade telemetry,
